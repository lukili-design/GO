...
import { 
  ...
  ExternalLink, Laptop, Monitor, Sliders, Database
} from 'lucide-react';
...
export const WorkspaceShell: React.FC = () => {
  ...
  const [bookings, setBookings] = useState<Booking[]>(() => {
    const saved = localStorage.getItem('tvb_go_bookings_v3');
    if (saved) {
      try {
  ...
  // Workspace view toggle: 'DUAL' (split screen phone + sandbox) | 'CMS' (full administration portal)
  const [workspaceMode, setWorkspaceMode] = useState<'DUAL' | 'CMS'>('DUAL');
  ...
  useEffect(() => {
    localStorage.setItem('tvb_go_bookings_v3', JSON.stringify(bookings));
  }, [bookings]);
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col">
      ...
      <div className="bg-slate-150/80 dark:bg-slate-900/45 border-b border-slate-200 dark:border-slate-800 p-2 flex justify-center">
        <div className="bg-white dark:bg-slate-950 p-1 rounded-2xl border border-slate-200 dark:border-slate-800/80 flex shadow-xs gap-1 max-w-xl w-full">
          <button
            onClick={() => {
      ...
      <main className="flex-1 p-4 md:p-6 max-w-7xl mx-auto w-full flex flex-col">
        {workspaceMode === 'CMS' ? (
          ...
          <div className="flex flex-col items-center justify-center w-full py-4">
            ...
            <div className="w-full max-w-[375px] h-[720px] bg-slate-950 rounded-[48px] p-3.5 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] border-4 border-slate-800/90 relative flex flex-col">
